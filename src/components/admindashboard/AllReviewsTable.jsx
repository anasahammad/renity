...
const AllReviewsTable = ({ reviews, refetch }) => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, newStatus }) => axiosInstance.put(`/review/${id}`, newStatus),
    onSuccess: () => {
  ...
  const filteredUsers = reviews.filter((review) => review?.name.toLowerCase().includes(searchTerm.toLowerCase()) || review?.email.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return (
    <div className='container mx-auto p-2 md:p-6'>
      <h1 className='text-2xl font-semibold mb-4'>Reviews ({reviews?.length})</h1>
      ...
      <div className='overflow-x-auto bg-white shadow-md rounded-lg'>
        <table className='min-w-full leading-normal'>
          ...
          <tbody>
            {currentUsers.map((review) => (
              <tr key={review.id}>
                <td className='px-5 py-5 border-b border-gray-200 bg-white text-sm'>
                  <div className='flex items-center'>
                    <div className='flex-shrink-0 w-10 h-10'>
                      <img className='w-full h-full rounded-full' src={`https://ui-avatars.com/api/?name=${review?.name}&background=random`} alt='' />
                    </div>
                    <div className='ml-3'>
                ...
                <td className='px-5 py-5 border-b border-gray-200 bg-white text-sm'>
                  <p className='text-gray-900 whitespace-no-wrap'>{review.email}</p>
                </td>
                <td className='px-5 py-5 border-b border-gray-200 bg-white text-sm'>
      ...
      <div className='mt-4 flex flex-wrap items-center justify-between'>
        <div className='w-full sm:w-auto mb-4 sm:mb-0'>
          <p className='text-sm text-gray-700'>
            Showing {indexOfFirstUser + 1} to {Math.min(indexOfLastUser, filteredUsers.length)} of {filteredUsers.length} reviews
          </p>
        </div>
