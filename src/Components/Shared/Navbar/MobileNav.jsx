import { FaBars, FaCaretRight, FaArrowLeft } from "react-icons/fa";
        ...
        <>
                        ...
                        {navItems.map((navItem, index) => (
                            <li className="my-4" key={index}>
                                <div className="flex justify-between items-center">
                                    ...
                                    {navItem.subMenu && (
                                        <FaCaretRight onClick={() => handleSubMenuOpen(index)} />
                                    )}
                                </div>
                            </li>
                        ))}
                        ...
                        {navItems[openSubMenuIndex].subMenu.map((subItem, subIndex) => (
                            <li className="my-2" key={subIndex}>
                                <div className="flex justify-between items-center">
                                    ...
                                    {subItem.subItem && (
                                        <FaCaretRight onClick={() => handleNestedSubMenuOpen(subIndex)} />
                                    )}
                                </div>
                            </li>
                        ))}
            ...
            {openNestedSubMenuIndex !== null && (
                <div className="bg-[#F5AB1A] text-gray-200 pl-4 py-2">
                    ...
                    <ul>
                        {navItems[openSubMenuIndex].subMenu[openNestedSubMenuIndex].subItem.map(
