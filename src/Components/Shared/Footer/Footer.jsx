    ...
    <footer className="bg-[#FBFAF3] py-20">
      ...
      <div className='px-8 pt-20 flex flex-col md:flex-row items-center justify-between'>
        <div className='text-gray-600 text-sm'>
            &copy; {new Date().getFullYear()} Renity Rental. All Rights Reserved. Terms & Conditions
        </div>
        
        {/* Social Links */}
        <div className='flex items-center gap-3 md:gap-4'>
            {socialMedia.map((item, index)=><Link to={item.path} className='bg-white p-3 rounded-full' key={index}>{item.icon}</Link>)}
        </div>
      </div>
    </footer>
  );
