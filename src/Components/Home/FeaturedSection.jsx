...
const FeaturedSection = () => {
 ...
 <div className="relative border-b  h-[420px] group mb-4">
                <div style={{ backgroundImage: `url(${feature.image})` }} className="  flex justify-center items-center  bg-cover w-full h-[295px] bg-center cursor-pointer  ">

                </div>

                <div className="block group-hover:hidden px-1 py-4">
                    <h2 className="text-2xl font-semibold">{feature.name}</h2>
                    <p className="text-gray-600 text-sm">Rent Per Day <span className="text-[#F8748C] text-xl">${feature.rent}</span></p>
                </div>
                ...
                <p className="text-gray-600 text-sm my-4 border-b pb-4">Rent Per Day <span className="text-[#F8748C] text-xl">${feature.rent}</span></p>

                <ul className="list-disc pl-4 text-sm font-semibold">
                    {feature.features.map((item, idx)=><li key={idx}>{item}</li>)}
