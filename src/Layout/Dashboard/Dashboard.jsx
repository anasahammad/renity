...
const Dashboard = () => {
  ...
  <NavLink
    ...
    to={item.path}
    className={({ isActive }) =>
      `flex items-center gap-3 px-4 py-3 rounded-lg font-semibold text-white ${
        isActive ? "bg-[#ff4d30]" : "hover:bg-[#ff4d30]"
      }`
    }
  >
    <span>{item.icon}</span>
    {item.label}
            ...
            <div className=""><button className=" py-2  pl-4 w-full rounded-lg text-[18px] text-white text-left flex items-center gap-4 hover:bg-[#ff4d30]"><HiOutlineLogout /> Logout</button></div>
          </nav>
        </div>
      </aside>
...
<div className="cursor-pointer relative"><IoIosNotificationsOutline  size={24} />
<span className="absolute h-5 w-5 bg-[#ff4d30] rounded-full -top-2 -left-1 text-white flex items-center justify-center text-sm">1</span>
          ...
          {isDropdownOpen && (<div className="w-52 absolute right-4  bg-white rounded-sm p-4 shadow-md ">
           <div className="pl-2 mb-2">Anas Ahammad Sarker</div>
           <hr />
               <ul className="space-y-2  mt-2">
               
                <li><button className="flex items-center gap-2"><HiOutlineLogout className="mt-1"/> Logout</button></li>
               </ul>
             </div>)}
        </header>
